...
@type function
	...
	@param cTipoPar,  characters, Tipo do ParÃ¢metro        (G = Get, C = Combo, F = Escolha de Arquivos, K = Check Box)
	...
	@param cPicture,  characters, Máscara do ParÃ¢metro     (ex.: @!, @E 999.99, ...)
	...
	@param cHelp,     characters, Texto de Help do parÃ¢metro
	@obs Função foi criada, pois a partir de algumas versÃµes do Protheus 12, a função padrão PutSX1 não funciona (por medidas de segurança)
...
User Function zPutSX1(cGrupo, cOrdem, cTexto, cMVPar, cVariavel, cTipoCamp, nTamanho, nDecimal, cTipoPar, cValid, cF3, cPicture, cDef01, cDef02, cDef03, cDef04, cDef05, cHelp)
	...
	//Se tiver Grupo, Ordem, Texto, ParÃ¢metro, Variável, Tipo e Tamanho, continua para a criação do parÃ¢metro
	If !Empty(cGrupo) .And. !Empty(cOrdem) .And. !Empty(cTexto) .And. !Empty(cMVPar) .And. !Empty(cVariavel) .And. !Empty(cTipoCamp) .And. nTamanho != 0
		...
		nDecimal   := Iif(cTipoPar == "N", nDecimal, 0)               //Se não for parÃ¢metro do tipo numérico, será 0 o Decimal
		...
		cTipoPar   := Upper(cTipoPar)                                 //Deixa o tipo do ParÃ¢metro em maiúsculo
		cTipoPar   := Iif(Empty(cTipoPar), 'G', cTipoPar)             //Se o tipo do ParÃ¢metro estiver em branco, será um Get
