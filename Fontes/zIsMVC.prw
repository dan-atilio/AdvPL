...
@version 1.0
@type function
@obs Essa função, apenas pega e verifica se existe um modelo de dados ativo aberto, caso exista, ela mostra uma tela com observaçÃµes
...
User Function zIsMVC()
	...
	If oModelPad != Nil
		...
		DEFINE MSDIALOG oDlgPvt TITLE "zIsMVC" FROM 000, 000  TO nJanAltu, nJanLarg COLORS 0, 16777215 PIXEL
			...
			@ 056, 006 BUTTON oBtnL001  PROMPT "VÃ­deo Aula - Conceitos de MVC em AdvPL"               SIZE 285, 016 FONT oFontSub   OF oFolderPvt:aDialogs[1] ACTION (fLink(1))       PIXEL
			...
			@ 116, 006 BUTTON oBtnL004  PROMPT "VÃ­deo Aula - Como criar um ponto de entrada em MVC"   SIZE 285, 016 FONT oFontSub   OF oFolderPvt:aDialogs[1] ACTION (fLink(4))       PIXEL
			@ 136, 006 BUTTON oBtnL005  PROMPT "VÃ­deo Aula - Como criar ExecAuto em MVC"              SIZE 285, 016 FONT oFontSub   OF oFolderPvt:aDialogs[1] ACTION (fLink(5))       PIXEL
	...
	Else
		MsgAlert("Não há indÃ­cios dessa tela ser em MVC (utilize dentro de uma tela de Inclusão/Alteração/Exclusão)!", "Atenção")
	EndIf
...
Static Function fLink(nBotao)
	...
	//VÃ­deo Aula - Conceitos de MVC em AdvPL
	If nBotao == 1
	...
	//VÃ­deo Aula - Como criar um ponto de entrada em MVC
	ElseIf nBotao == 4
		cLink := "https://terminaldeinformacao.com/2017/01/16/vd-advpl-023/"
	
	//VÃ­deo Aula - Como criar ExecAuto em MVC
	ElseIf nBotao == 5
...
Static Function fPontoEnt(cID)
	...
	cPonto += cTab + ' ' + cQuebra
	cPonto += cTab + cAbreComent + '//Se tiver parÃ¢metros' + cFechComent + cQuebra
	cPonto += cTab + 'If aParam != Nil ' + cQuebra
	cPonto += cTab + cTab + 'ConOut("> "+aParam[2]) ' + cQuebra
	cPonto += cTab + cTab + ' ' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Pega informaçÃµes dos parÃ¢metros' + cFechComent + cQuebra
	...
	cPonto += cTab + cTab + cTab + ' ' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Pré configuraçÃµes do Modelo de Dados' + cFechComent + cQuebra
	cPonto += cTab + cTab + 'ElseIf cIdPonto == ' + cAbreString + '"MODELPRE"' + cFechString + ' ' + cQuebra
	cPonto += cTab + cTab + cTab + 'xRet := .T. ' + cQuebra
	cPonto += cTab + cTab + cTab + ' ' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Pré configuraçÃµes do Formulário de Dados' + cFechComent + cQuebra
	...
	cPonto += cTab + cTab + cTab + cAbreComent + '//EndIf' + cFechComent + cQuebra
	cPonto += cTab + cTab + cTab + '' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Adição de opçÃµes no AçÃµes Relacionadas dentro da tela' + cFechComent + cQuebra
	...
	cPonto += cTab + cTab + cTab + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Pós configuraçÃµes do Formulário' + cFechComent + cQuebra
	...
	cPonto += cTab + cTab + cTab + cAbreComent + '//EndIf' + cFechComent + cQuebra
	cPonto += cTab + cTab + cTab + ' ' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Pré validaçÃµes do Commit' + cFechComent + cQuebra
	cPonto += cTab + cTab + 'ElseIf cIdPonto == ' + cAbreString + '"FORMCOMMITTTSPRE"' + cFechString + ' ' + cQuebra
	cPonto += cTab + cTab + cTab + ' ' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Pós validaçÃµes do Commit' + cFechComent + cQuebra
	cPonto += cTab + cTab + 'ElseIf cIdPonto == ' + cAbreString + '"FORMCOMMITTTSPOS"' + cFechString + ' ' + cQuebra
	cPonto += cTab + cTab + cTab + ' ' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Commit das operaçÃµes (antes da gravação)' + cFechComent + cQuebra
	cPonto += cTab + cTab + 'ElseIf cIdPonto == ' + cAbreString + '"MODELCOMMITTTS"' + cFechString + ' ' + cQuebra
	cPonto += cTab + cTab + cTab + ' ' + cQuebra
	cPonto += cTab + cTab + cAbreComent + '//Commit das operaçÃµes (após a gravação)' + cFechComent + cQuebra
