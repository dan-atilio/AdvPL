...
WsMethod RetListCli WsReceive cFiltRece WsSend cFiltSend WsService zWsCliente
	...
	//ParÃ¢metros recebidos pelo JSON (variável cFiltRece)
	Local cParVend   := ""
	Local cParEst    := ""
	Local cParMun    := ""
	Local cParToken  := ""
	...
	If (FWJsonDeserialize(::cFiltRece, @oJSON))
		...
		//Se o Token recebido for o mesmo do parÃ¢metro, prossegue
		If cParToken == cTokWs
			
			//Pegando os 3 filtros possÃ­veis vindo dentro de "Dados"
			...
			//Se os 3 parÃ¢metros estiverem em branco, retorna erro, só pode prosseguir se pelo menos 1 estiver preenchido
			...
			Else
				...
				If ! QRY_CLI->(EoF())	
					...
					While ! QRY_CLI->(EoF())
						...
						//Se não for o último cliente, acrescenta a vÃ­rgula
						If ! QRY_CLI->(EoF())
							::cFiltSend += ','
						EndIf
						::cFiltSend += CRLF
