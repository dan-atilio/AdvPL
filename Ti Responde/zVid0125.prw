...
Função que demonstra como gatilhar uma informação de uma coluna para outra usando FWBrowse
@type  Function
@author Atilio
@since  11/03/2024
@version version
...
Static Function fMontaHead()
    ...
    aAdd(aHeadAux, {"XXPRODCOD", "Produto Cód.",  "C", TamSX3("B1_COD")[1],  0, "",              .T., 'SB1', {|| ExistCPO('SB1'), fAtuDesc()},  Nil})
...
Static Function fAtuDesc()
    ...
    //Se o produto não estiver posicionado, posiciona a tabela
    If cCodProd != SB1->B1_COD
        lContinua := SB1->(MsSeek(FWxFilial('SB1') + cCodProd))
    EndIf

    //Se deu tudo certo, pega a descrição do produto
    If lContinua
