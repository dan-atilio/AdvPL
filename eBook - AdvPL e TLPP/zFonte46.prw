...
/*/{Protheus.doc} zFonte46
Exemplo de geração de relatório com FWMSPrinter (impressão de imagens com SayBitmap)
@author Atilio
@since 28/11/2022
@version 1.0
@type function
...
Static Function fMontaRel()
    ...
    If lJob
        ...
        cArquivo := "zFonte46_job_" + dToS(dDataGer) + "_" + StrTran(cHoraGer, ':', '-') + ".pdf"
    ...
    Else
        ...
        cArquivo  := "zFonte46_" + dToS(dDataGer) + "_" + StrTran(cHoraGer, ':', '-')
    ...
    nLinAtu  := 40
    oPrintPvt:SayAlign(nLinAtu, nColIni      , "Dados:",                            oFontDetN,  200,    015, , PAD_LEFT,  )
    nLinAtu += nEspLin

    //Imprimindo o logo
    cLogoRel := "\x_imagens\logo.png"
    oPrintPvt:SayBitmap(nLinAtu, nColIni + 065, cLogoRel, 070, 070) // 100 é a metade de 200, - 35 que é a metade da largura, dá 065
